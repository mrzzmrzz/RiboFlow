...
from src.deps.grnade.src.constants import NUM_TO_LETTER
from src.deps.grnade.src.data.data_utils import get_c4p_coords
from src.deps.grnade.src.data.sec_struct_utils import (
...
def train(config, model, train_loader, val_loader, test_loader, device):
        ...
        if epoch % config.val_every == 0 or epoch == config.epochs - 1:
            ...
            with torch.no_grad():
                ...
                if val_acc > best_val_acc:
                    ...
                    print_and_log(
                        epoch, test_loss, test_acc, test_confusion, mode="test", lookup=lookup
                    )
    ...
    if config.save:
        ...
        print(
            f"BEST VAL recovery: {np.mean(val_recovery_list):.4f} scscore: {np.mean(val_scscore_list):.4f}"
        )
        torch.save(
        ...
        print(
            f"BEST TEST recovery: {np.mean(test_recovery_list):.4f} scscore: {np.mean(test_scscore_list):.4f}"
        )
        torch.save(
...
def evaluate(model, dataset, n_samples, temperature, device, model_name="eval"):
    """
    ...
    model.eval()
    with torch.no_grad():
        for idx, raw_data in tqdm(enumerate(dataset.data_list), total=len(dataset.data_list)):
            # featurise raw data
            ...
            sc_score = self_consistency_score(
                samples.cpu().numpy(), raw_data["sec_struct_list"], mask_coords
            )
...
def self_consistency_score_ribonanza(
    samples, true_chem_mod, ribonanza_net, num_to_letter=NUM_TO_LETTER, return_chem_mods=False
):
    """
