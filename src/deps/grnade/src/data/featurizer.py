...
from src.deps.grnade.src.constants import DISTANCE_EPS, RNA_ATOMS, RNA_NUCLEOTIDES
from src.deps.grnade.src.data.data_utils import (
    get_backbone_coords,
    get_k_random_entries_and_masks,
    pdb_to_tensor,
)
...
class RNAGraphFeaturizer(object):
    ...
    def __call__(self, rna):
        with torch.no_grad():
            ...
            seq = torch.as_tensor(
                [0 for residue in rna["sequence"]], device=self.device, dtype=torch.long
            )
            ...
            dihedrals, angles, lengths = internal_coords(
                coords_list, mask_coords.unsqueeze(0).expand(self.max_num_conformers, -1)
            )
            ...
            edge_posenc = (
                positional_encoding((edge_index[0] - edge_index[1])[..., None], self.num_posenc)
                .unsqueeze_(1)
