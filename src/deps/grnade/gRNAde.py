...
from src.deps.grnade.src.constants import (
    FILL_VALUE,
    # PROJECT_PATH
    NUM_TO_LETTER,
    RNA_ATOMS,
)
from src.deps.grnade.src.data.data_utils import get_backbone_coords

# from src.trainer import self_consistency_score
from src.deps.grnade.src.data.featurizer import RNAGraphFeaturizer
from src.deps.grnade.src.models import AutoregressiveMultiGNNv1
...
CHECKPOINT_PATH = {
    "all": {
        1: os.path.join("src/deps/grnade/checkpoints/gRNAde_ARv1_1state_all.h5"),
...
class gRNAde(object):
    ...
    def design_from_pdb_file(
        ...
        return self.design(
            raw_data, featurized_data, output_filepath, n_samples, temperature, partial_seq, seed
        )
    ...
    def design(
        ...
        for idx, zipped in enumerate(zip(samples.cpu().numpy(), perplexity, recovery)):
            seq, perp, rec = zipped
    ...
    def perplexity(
        ...
        # compute perplexity
        perplexity = (
            torch.exp(F.cross_entropy(logits / temperature, _seq, reduction="none").mean())
            .cpu()
            .numpy()
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seed", dest="seed", default=0, type=int, help="Random seed for reproducibility"
    )
    parser.add_argument(
        "--gpu_id",
        dest="gpu_id",
    ...
    g = gRNAde(split=args.split, max_num_conformers=args.max_num_conformers, gpu_id=args.gpu_id)
