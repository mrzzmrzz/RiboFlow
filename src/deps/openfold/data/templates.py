...
from src.deps.openfold.data import mmcif_parsing, parsers
from src.deps.openfold.data.errors import Error
from src.deps.openfold.data.tools import kalign
from src.deps.openfold.data.tools.utils import to_date
from src.deps.openfold.np import residue_constants
...
def get_custom_template_features(
    ...
    for template_path in template_paths:
        ...
        curr_features["template_sum_probs"] = [1.0]  # template given by user, 100% confident
        template_features = {
    ...
    template_features = {
        template_feature_name: np.stack(template_features[template_feature_name], axis=0).astype(
...
class HmmsearchHitFeaturizer(TemplateHitFeaturizer):
    def get_templates(
        ...
        filtered = list(
            sorted(filtered, key=lambda x: x.sum_probs if x.sum_probs else 0.0, reverse=True)
        )
        idx = list(range(len(filtered)))
        if self._shuffle_top_k_prefiltered:
            stk = self._shuffle_top_k_prefiltered
