...
from src.deps.openfold.config import NUM_RES
from src.deps.openfold.data.data_transforms import curry1
from src.deps.openfold.np import residue_constants as rc
from src.deps.openfold.utils.tensor_utils import masked_mean
...
def create_msa_feat(batch):
    ...
    deletion_mean_value = (torch.atan(batch["cluster_deletion_mean"] / 3.0) * (2.0 / pi))[
        ..., None
    ]
...
def get_spatial_crop_idx(protein, crop_size, interface_threshold, generator):
    ...
    break_tie = (
        torch.arange(0, to_target_distances.shape[-1], device=positions.device).float() * 1e-3
    )
...
def random_crop_to_size(
    ...
    use_spatial_crop = (
        torch.rand((1,), device=protein["seq_length"].device, generator=g) < spatial_crop_prob
    )
    ...
    if subsample_templates:
        templates_crop_start = randint(
            lower=0, upper=num_templates, generator=g, device=protein["seq_length"].device
        )
