...
def get_least_asym_entity_or_longest_length(
    ...
    unique_entity_ids = [
        i for i in torch.unique(batch["entity_id"]) if i != 0
    ...
    anchor_pred_asym_ids = [
        asym_id for asym_id in entity_2_asym_list[least_asym_entities] if asym_id in input_asym_id
    ]
...
def compute_permutation_alignment(
    ...
    for candidate_pred_anchor in anchor_pred_asym_ids:
        ...
        merged_labels = merge_labels(
            per_asym_residue_index, labels, align, original_nres=features["aatype"].shape[-1]
        )
        rmsd = compute_rmsd(
