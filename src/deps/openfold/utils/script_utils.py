...
from src.deps.openfold.model.model import AlphaFold
from src.deps.openfold.np import protein, residue_constants
from src.deps.openfold.np.relax import relax
from src.deps.openfold.utils.import_weights import import_jax_weights_, import_openfold_weights_
...
def run_model(model, batch, tag, output_dir):
    with torch.no_grad():
        ...
        update_timings(
            {tag: {"inference": inference_time}}, os.path.join(output_dir, "timings.json")
        )
...
def prep_output(
    out, batch, feature_dict, feature_processor, config_preset, multimer_ri_gap, subtract_plddt
):
    plddt = out["plddt"]
...
def relax_protein(
    config, model_device, unrelaxed_protein, output_directory, output_name, cif_output=False
):
