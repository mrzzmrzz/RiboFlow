...
from src.deps.rhofold.rhofold.utils.constants import RNA_CONSTANTS
from src.deps.rhofold.rhofold.utils.rigid_utils import (
    Rigid,
    calc_angl_rot_tsl,
    calc_rot_tsl,
    merge_rot_tsl,
...
class RNAConverter:
    ...
    def build_cords(self, seq, fram, angl, rtn_cmsk=False):
        ...
        amsk = torch.ones(
            (n_resds, RNA_CONSTANTS.N_ANGLS_PER_RESD_MAX), dtype=torch.int8, device=device
        )
        cord = torch.zeros(
        ...
        for resd_name in RNA_CONSTANTS.RESD_NAMES:
            ...
            if len(idxs) == 0:
                continue
            cord[idxs], cmsk[idxs] = self.__build_cord(
    ...
    def __build_cord(self, resd_name, fram, fmsk, angl, amsk):
        ...
        for idx_rgrp, rgrp_name_curr in enumerate(rgrp_names_all):
            ...
            fram_dict[rgrp_name_curr] = torch.cat([rot_curr, tsl_curr.unsqueeze(dim=1)], dim=1)
        ...
        cmsk = torch.stack(
            [cmsk_vec_dict[x] for x in atom_names_pad][: RNA_CONSTANTS.ATOM_NUM_MAX], dim=1
        )
        cord = torch.stack(
    ...
    def export_pdb_file(
        self, seq, atom_cords, path, atom_masks=None, confidence=None, chain_id=None, logger=None
    ):
        """Export a PDB file."""
        ...
        with open(path, "w") as o_file:
            ...
            for idx_resd, resd_name in enumerate(seq):
                for idx_atom, atom_name in enumerate(
                    RNA_CONSTANTS.ATOM_NAMES_PER_RESD[resd_name]
                ):
                    temp_factor = (
                        0.0
