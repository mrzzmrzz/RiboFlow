...
class CrossAttention(nn.Module):
    ...
    def forward(
        self,
        query_input,  # [B, Q, query_input_dim]
        key_input,  # [B, K, key_input_dim]
        value_input,  # [B, K, key_input_dim]
        query_input_mask,  # [B, N]
        key_input_mask,  # [B, N]
    ):
        query = self.W_Q(query_input)  # [B, Q, output_dim]
        key = self.W_K(key_input)  # [B, K, output_dim]
        value = self.W_V(value_input)  # [B, K, output_dim]

        # [B, Q, output_dim] *  [B, output_dim, K] ->  # [B, Q, K]
        ...
        attn_weights = attn_weights.masked_fill(~attn_mask, -1e9)  # [B, Q, K]
        attn_weights = self.softmax(attn_weights)  # [B, Q, K]

        # [B, Q, K] * [B, K, output_dim] -> [B, Q, output_dim]
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads, embed_dim, bias=False):
...
class Encoder(nn.Module):
    def __init__(self, n_layers, n_heads, model_depth, ff_depth, dropout):
        ...
        self.layers = nn.ModuleList(
            [EncoderLayer(n_heads, model_depth, ff_depth, dropout) for i in range(n_layers)]
        )
...
class EncoderLayer(nn.Module):
    def __init__(self, n_heads, model_depth, ff_depth, dropout=0.0):
...
class PositionalEncoding(nn.Module):
    def __init__(self, model_depth, max_len=5000):
        ...
        div_term = torch.exp(
            torch.arange(0.0, model_depth, 2) * -(math.log(10000.0) / model_depth)
        )
