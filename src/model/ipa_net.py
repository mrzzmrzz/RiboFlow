...
from src.data.basics.utils import Rigid
from src.model.positional_embeddings import PositionalEmbedding
from src.model.torsion_net import TorsionAngleHead
...
def compute_angles(ca_pos, pts):
    ...
    phi_angles = all_atom_na.calculate_neighbor_angles(
...
class BackboneUpdate(nn.Module):
    ...
    def __init__(self, c_s, use_rot_updates=True):
        """
        Args:
            c_s:
                Single representation channel dimension
...
class IPANet(nn.Module):
    def __init__(self, IPA_conf):
        ...
        """
        super(IPANet, self).__init__()
        self.ipa_conf = IPA_conf

        # self.scale_pos = lambda x: x * self.ipa_conf.coordinate_scaling
        # self.scale_rigids = lambda x: x.apply_trans_fn(self.scale_pos)

        # self.unscale_pos = lambda x: x / self.ipa_conf.coordinate_scaling
        ...
        for b in range(self.ipa_conf.ipa.num_blocks):
            self.trunk[f"ipa_{b}"] = InvariantPointAttention(self.ipa_conf.ipa)
            self.trunk[f"ipa_ln_{b}"] = nn.LayerNorm(self.ipa_conf.ipa.c_s)
            tfmr_in = self.ipa_conf.ipa.c_s
            tfmr_layer = nn.TransformerEncoderLayer(
                d_model=tfmr_in,
                nhead=self.ipa_conf.ipa.seq_tfmr_num_heads,
            ...
            if b < self.ipa_conf.ipa.num_blocks - 1:
                # No edge update on the last block.
                edge_in = self.ipa_conf.edge_embed_dim
                self.trunk[f"edge_transition_{b}"] = EdgeTransition(
                    node_embed_size=self.ipa_conf.ipa.c_s,
                    edge_embed_in=edge_in,
                    edge_embed_out=self.ipa_conf.edge_embed_dim,
                )

        self.torsion_pred_net = TorsionAngleHead(
            c_in=self.ipa_conf.ipa.c_s,
            c_hidden=128,
            no_blocks=2,
            no_angles=8,
            epsilon=1e-12,
    ...
    def forward(self, init_node_embed, init_edge_embed, input_feats, use_context=False):
        ...
        for b in range(self.ipa_conf.ipa.num_blocks):
            ...
            seq_tfmr_out = self.trunk[f"seq_tfmr_{b}"](
                node_embed,
                src_key_padding_mask=(1 - node_mask).bool(),
            )
        ...
        _, pred_torsions = self.torsion_pred_net(node_embed, init_node_embed)
