...
from src.data.basics import rigid_utils_na as ru
from src.data.basics.constants import nucleotide_constants, protein_constants
from src.data.basics.rigid_utils_na import Rigid
...
NM_TO_ANG_SCALE = 10.0  # nanometers (nm) -> angstroms (Å)
ANG_TO_NM_SCALE = 1 / NM_TO_ANG_SCALE  # angstroms (Å) -> nanometers (nm)

NUM_TOKENS = protein_constants.restype_num  # 20
MASK_TOKEN_INDEX = protein_constants.restypes_with_x.index("X")  # 20
CA_IDX = protein_constants.atom_order["CA"]
...
# NOTE: `23` comes from the maximum number of atoms in a nucleic acid
MAX_NUM_ATOMS_PER_RESIDUE = 23
...
# Note: the order is as follows:
# (protein_feature_name, na_feature_name, complex_feature_name, padding_dim): max_feature_dim_size
...
# aatype_to_seq = lambda aatype: "".join([protein_constants.restypes_with_x[x] for x in aatype])
...
def aatype_to_seq(aatype):
    return "".join([protein_constants.restypes_with_x[x] for x in aatype])
...
def pad(x: np.ndarray, max_len: int, pad_idx=0, reverse=False):
...
def pad_feats(raw_feats, max_len):
    ...
    for feat_name in RIGID_FEATS:
        if feat_name in raw_feats:
            padded_feats[feat_name] = pad_rigid(raw_feats[feat_name], max_len).numpy()
    return padded_feats
...
def parse_chain_feats_pdb(
    chain_feats: Dict[str, Any],
    molecule_constants: ModuleType,
    molecule_backbone_atom_name: str,
    scale_factor: float = 1.0,
) -> Dict[str, Any]:
    """
    Parse and process chain features from PDB data by centering and scaling atomic positions.

    Args:
        chain_feats: Dictionary containing chain features with keys:
            - 'atom_mask': Binary mask indicating valid atoms [num_residues, num_atoms]
            - 'atom_positions': Atomic coordinates [num_residues, num_atoms, 3]
        molecule_constants: ModuleType containing molecular constants with attribute:
            - atom_order: Dictionary mapping atom names to indices
        molecule_backbone_atom_name: Name of the backbone atom to use as reference
        scale_factor: Factor to scale atomic positions (default: 1.0)

    Returns:
        Dict[str, Any]: Processed chain features with additional keys:
            - 'bb_mask': Binary mask for backbone atoms
            - 'bb_positions': Coordinates of backbone atoms
            - 'atom_positions': Centered and scaled atomic positions

    Note:
        The function performs the following operations:
        1. Extracts backbone atom masks and positions
        2. Centers all atomic positions relative to backbone center
        3. Scales positions by the provided scale factor
        4. Applies atom mask to scaled positions
    """
    # Get index of the backbone atom (NA:C4' protein:CA)
    ...
    # Extract and center backbone positions
    bb_pos = chain_feats["atom_positions"][:, core_atom_idx]
    chain_feats["bb_positions"] = bb_pos
    bb_center = np.sum(bb_pos, axis=0) / (np.sum(chain_feats["bb_mask"]) + 1e-5)  # [3]
    # Center and scale all atomic positions
    centered_pos = chain_feats["atom_positions"] - bb_center[None, None, :]
    centered_pos = centered_pos / scale_factor * chain_feats["atom_mask"][..., None]
    # Apply atom mask and update positions
    chain_feats["centered_atom_positions"] = centered_pos
    chain_feats["bb_center"] = bb_center
    return chain_feats
...
def concat_np_features(
    ...
    for feat_name, feat_vals in combined_dict.items():
        combined_dict[feat_name] = np.concatenate(feat_vals, axis=0).tolist()
...
def align_structures(
    ...
    batch_positions = center_zero(batch_positions, torch.LongTensor(batch_indices))
    reference_positions = center_zero(reference_positions, torch.LongTensor(batch_indices))
...
def concat_complex_torch_features(
    ...
    feature_concat_map: Mapping[Tuple[str, str, str, int], int],
    add_batch_dim: bool,
) -> Dict[str, torch.Tensor]:
    """Concatenates protein and nucleic acid features into complex features with padding as needed.

    This function combines features from protein and nucleic acid tensors into a single complex
    feature tensor. It handles padding to ensure consistent dimensions and can optionally add
    a batch dimension to the output.

    Args:
        complex_torch_dict: Dictionary to store the concatenated complex features.
        protein_torch_dict: Dictionary containing protein features as torch tensors.
        na_torch_dict: Dictionary containing nucleic acid features as torch tensors.
        feature_concat_map: Mapping defining how features should be concatenated.
            Keys are tuples of (protein_feature_name, na_feature_name, complex_feature_name, padding_dimension).
            Values are the maximum feature dimension sizes for padding.
        add_batch_dim: If True, adds a batch dimension to each complex feature.

    Returns:
        Dict[str, torch.Tensor]: Updated complex_torch_dict containing the concatenated features.

    Raises:
        Exception: If neither protein nor nucleic acid features are provided for a required concatenation.

    Example:
        feature_map = {
            ('protein_pos', 'na_pos', 'complex_pos', 0): 100
        }
        result = concat_complex_torch_features(
            complex_dict={},
            protein_dict={'protein_pos': tensor},
            na_dict={'na_pos': tensor},
            feature_concat_map=feature_map,
            add_batch_dim=True
        )
    """
    for (
        ...
        # parse available protein and NA features
