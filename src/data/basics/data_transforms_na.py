...
from src.data.basics.constants import nucleotide_constants as nc
from src.data.basics.constants import vocabulary
from src.data.basics.constants.complex_constants import NUM_PROT_NA_TORSIONS
from src.data.basics.rigid_utils_na import Rigid, Rotation
from src.deps.openfold.data.data_transforms import batched_gather, curry1
...
def convert_na_aatype6_to_aatype9(
    ...
    aatype[aatype == nc.NA_AATYPE6_MASK_RESIDUE_INDEX] = nc.NA_AATYPE9_MASK_RESIDUE_INDEX
...
def atom27_to_frames(na, eps=1e-8):
    ...
    nttype_rigidgroup_base_atom27_idx = nttype_rigidgroup_base_atom27_idx.view(
        *((1,) * batch_dims),
        *nttype_rigidgroup_base_atom27_idx.shape,
    )
...
def atom27_to_torsion_angles(
    na,
    prefix="",
    randomly_noise_torsion_atoms_to_place=False,
):
    ...
    assert 21 <= aatype.max() <= 26, "Only NA inputs are allowed in `atom27_to_torsion_angles()`."

    # FIXME: is aatype9 neceaaary?
    aatype = convert_na_aatype6_to_aatype9(aatype, deoxy_offset_mask=na_deoxy)

    chi_atom_indices = torch.as_tensor(get_chi_atom_indices("NA"), device=aatype.device)

    atom_indices = chi_atom_indices[..., aatype, :, :]
    chis_atom_pos = batched_gather(  # [N_res, 10, 4, 3]
        all_atom_positions,
        atom_indices,
        -2,
    ...
    chi_angles_mask.append([0.0 for _ in range(NUM_PROT_NA_TORSIONS)])  # for UNK
    ...
    chi_angle_atoms_mask = torch.prod(  # N_res，10
    ...
    torsion_angles_sin_cos = (  # [N_res, 10, 2]
