...
from src.data.basics.molecule.molecule_constants import (
...
def get_atom_features(data: dict, atom_features_list=None):
    if atom_features_list is None:
        atom_features_list = [
            "atomic_num",
            "formal_charge",
            "degree",
    ...
    atom_features_search_list = []
    for attr in atom_features_list:
        atom_features_search_list.append(data.get(attr, None))
    return torch.tensor(np.array(atom_features_search_list), dtype=torch.int32).T


def get_edge_features(data: dict, edge_features_list=None):
    if edge_features_list is None:
        edge_features_list = ["bond_dir", "bond_type", "is_in_ring", "bond_length"]
    edge_features_search_list = []
    for attr in edge_features_list:
        edge_features_search_list.append(data.get(attr, None))
    return torch.tensor(np.array(edge_features_search_list), dtype=torch.float64).T
